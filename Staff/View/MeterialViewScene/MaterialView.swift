...
struct MaterialView: View {
    ...
    @State private var navigateToDetailPage: Int? = nil
            ...
            NavigationView{
                ...
                ZStack{
                    Color("app_background")
                        .edgesIgnoringSafeArea(.all)//设置导航栏标题颜色
                    VStack(alignment: .leading){
                        ...
                        List{
                            if selectedOption == 0{//人员列表
                                ForEach(0..<peoples.count, id: \.self){
                                    index in
                                    VStack{
                                        NavigationLink(
                                            destination: PeopleDetailView(people: self.peoples[index]).onAppear{
                                                self.showAddButton = false
                                            },
                                            tag: index,
                                            selection: self.$navigateToDetailPage){
                                                EmptyView()
                                        }
                                        PeopleCell(people: self.peoples[index])
                                        .onTapGesture {self.navigateToDetailPage = index}
                                        .contextMenu {
                                            Button(action: {self.callPeopleWithPhoneNumber(people: self.peoples[index])}){
                                                Text(self.phoneNumberIsValidate ? "拨打" : "号码有误")
                                                Image(systemName: "phone.arrow.up.right")
                                            }
                                            .disabled(!self.phoneNumberIsValidate)
                                            .onAppear{
                                                self.initContextMenu(people: self.peoples[index])
                                            }
                                            
                                            
                                            Button(action: {
                                                self.deleteCurrentPeople(people: self.peoples[index])
                                            }){
                                ...
                                ForEach(tags, id: \.self){
                                    ...
                                    Text(tag.name ?? "")
                                        .padding()
                                        .background(Color.init(tag.colorName ?? "defaultColor"))
